...
function activar(){
	...
	{
		window.open("iframebases.php?idprovedor="+document.form1.provedor.value+"&idoperador="+document.form1.operador.value, "interframe"); 
...
function plan(){
	...
	{
		window.open("iframebases.php?idprovedor="+document.form1.provedor.value+"&idoperador="+document.form1.operador.value+"&idplanes="+document.form1.planes.value, "interframe"); 
		//alert(document.form1.provedor.value);
	}
...
{	
	...
	{		
			...
			//echo "chip".$chip;
			$MinutoActual=$NuevaConexion->recuperaRegistro("select MinutoActual from chip where idChip=$chip and baja='1'",'MinutoActual');
			if($MinutoActual!='')
			{
			...
			{
				?>
				<script>alert("La cantidad de minutos actuales es nula \ndebe ajustar el chip")</script>
				<?
			}
	}
}
...
{
	$mensajeboton = "Asignacion de Chips a las bases";
	$estado = "Asignacion de Chips a las bases";
	$oculto = 2;
}
...
<?PHP
	$NuevaConexion->leermysql("select * from bases"); 
		...
		<a href="javascript:Abrir('ShowBases.php?base=<? echo $arrayprovedor[id_base];?>&numero=<? echo $imagen->numero();?>','bases','300','200','scrollbars=NO,Status=NO');">
		<? 
...
<p align="center">
	<input type="button" name="boton" id="boton" value="Ver Inventario de Bases" onclick="javascript:window.open('kardexbases.php','','toolbar=no,menubar=no,scrollbars=yes,resizable=no,top=0,left=0,width=750,height=650')">
</p>
</body>
</html>
<script language="JavaScript"> 

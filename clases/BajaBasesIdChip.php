...
class BajaBasesIdChip{
	...
	*/
	public function __construct($numerobase,$chip,$fecha){
	...
	public function Baja(){
		...
		$NuevaConexion->leermysql("select * from chip where idChip='$this->chip' and estado='0' and  baja ='1'");///
		...
		$sql1=" select 
				* 
				...
				and idDatosCliente = '$this->numerobase' 	
				and Estado = '1' 
				and Baja = '1'
				order by idUsuarioChip desc
			   limit 1
				";
		//echo $sql1;die();
		...
		$this->identificadorusuario	= $ArrayUsuariochip[idUsuarioChip];
  		$this->chipactual = $ArrayUsuariochip[Chip_idChip];
 ...
 //echo "UPDATE chip SET estado='1', MinutoActual='$restan',login='$this->login' where  idChip = '$this->chip'";die();
	$NuevaConexion->leermysql("UPDATE chip SET estado='1', MinutoActual='$restan',actualizado='$this->login' where  idChip = '$this->chipactual'");	
	//paso 02	
